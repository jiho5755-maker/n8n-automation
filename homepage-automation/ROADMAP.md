...
> 상태: 🔵 진행 중 | 시작일: 2026-02-24

### Task 001: 노션 DB 6개 생성
> 참고: `notion-db-schema.md`

- [ ] CS 문의 DB 생성 → UUID: `_______________`
- [ ] CS 템플릿 DB 생성 → UUID: `_______________`
- [ ] 리뷰 DB 생성 → UUID: `_______________`
- [ ] 콘텐츠 캘린더 DB 생성 → UUID: `_______________`
- [ ] 경쟁사 상품 DB 생성 → UUID: `_______________`
- [ ] 가격 이력 DB 생성 → UUID: `_______________`

### Task 002: n8n Credential 등록
> 서버: https://n8n.pressco21.com > Settings > Credentials

- [ ] 메이크샵 오픈 API (HTTP Header Auth)
- [ ] OpenAI API (GPT-4o-mini)
- [ ] 에어테이블 Personal Access Token
- [ ] 노션 API (기존 automation-project 재사용 가능)
- [ ] 네이버 커머스 API (App ID/Secret, bcrypt용)
- [ ] 텔레그램 봇 (기존 동작 중)

### Task 003: 알림톡 템플릿 등록 (수동, 검수 대기)
> 참고: `alimtalk-templates.md`
> ⚠️ 메이크샵 관리자 UI에서만 등록 가능 (API 자동화 불가)

- [ ] **F010용** 무통장 취소 알림 (`PRESSCO_CANCEL`) — **최우선 등록**
